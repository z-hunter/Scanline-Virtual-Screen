...
export function Knob({ value, min, max, step, label, onChange, disabled }: { value: number; min: number; max: number; step: number; label: string; onChange: (value: number) => void; disabled?: boolean }) {
  ...
  return <div className={`svs-knob knob ${disabled ? 'disabled' : ''}`} role="slider" aria-label={label} aria-valuemin={min} aria-valuemax={max} aria-valuenow={value} aria-disabled={disabled ? 'true' : undefined} tabIndex={disabled ? -1 : 0} style={{ '--knob-progress': `${ratio * 340}deg` } as React.CSSProperties}
