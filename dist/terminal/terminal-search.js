function isCaseSensitive(query) {
    return query !== query.toLowerCase();
}
function lineText(line, cols) {
    let text = '';
    const columns = [];
...
function searchBuffer(buffer, cols, rows, query, bufferKind) {
    if (!query)
...
export function findTerminalMatches(terminal, query) {
