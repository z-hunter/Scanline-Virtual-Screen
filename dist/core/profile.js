...
export function profileFromRenderSettings(settings, modeId) {
    const crt = { ...settings };
...
export function defaultScreenProfile(modeId = '1024x768') {
...
function normalizeRenderSettings(source, fallback) {
    const result = { ...fallback };
    for (const [key, [min, max]] of Object.entries(ranges)) {
        const current = result[key];
        const value = key === 'scanlineCount' || key === 'bezelThickness'
            ? integerValue(source[key], current, min, max)
            : numberValue(source[key], current, min, max);
        result[key] = value;
    }
    for (const key of ['bezelGlow', 'showBezel', 'reflexBarEnabled', 'channelSwitchEffect', 'antiAliasedPixels']) {
        if (typeof source[key] === 'boolean')
            result[key] = source[key];
    }
    if (source.bezelGlowMode === 'spill' || source.bezelGlowMode === 'reflection')
        result.bezelGlowMode = source.bezelGlowMode;
    if (source.colorMode === 'color' || source.colorMode === 'bw' || source.colorMode === 'green' || source.colorMode === 'green-p39' || source.colorMode === 'amber' || source.colorMode === 'blue')
        result.colorMode = source.colorMode;
    if (source.maskType === 'off' || source.maskType === 'aperture' || source.maskType === 'slot' || source.maskType === 'shadow')
        result.maskType = source.maskType;
    if (source.bloomAlgorithm === 'soft' || source.bloomAlgorithm === 'spiral')
        result.bloomAlgorithm = source.bloomAlgorithm;
    if (typeof source.crtEmulation === 'boolean')
        result.crtEmulation = source.crtEmulation;
    return result;
}
function normalizeTerminalSettings(source, fallback) {
    ...
    if (typeof font === 'string' && font.length > 0 && font.length <= 128)
        result.fontFamily = font;
    const fallbackFont = source.fallbackFont ?? source.FallbackFont;
    if (typeof fallbackFont === 'string' && fallbackFont.trim().length <= 128)
        result.fallbackFont = fallbackFont.trim() || undefined;
...
export function normalizeProfile(input, fallback = defaultScreenProfile(), availableModes = []) {
    ...
    if (!legacy && !current)
        return null;
    const sourceCrt = value.crt;
    const sourceTerminal = (current ? value.terminal : sourceCrt);
    const requestedMode = current ? value.virtualScreen.modeId : value.resolution;
...
function terminalToRender(terminal) {
    return {
...
export function profileToJSON(profile) { return JSON.stringify(profile); }
export function profileFromLegacyPreset(preset, availableModes = []) {
...
export function legacyPresetFromProfile(profile) {
